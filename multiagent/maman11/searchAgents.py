...
def cornersHeuristic(state, problem):
  ...
  for p in corners:
      ...
      if True == goal[j]:
         ...
         for c in corners :
             ...
             if node != c :
               dis=util.manhattanDistance(node,c)
               if min>dis and False == goal[i] and not (i in corner):
                  min=dis
                  index=i
             i+=1
         corner.append(index)
  ...
  return  goalCount # Default to trivial solution
...
def foodHeuristic(state, problem):
  ...
  for x in range(0, cnt):
    pPoint=disFood(position,foodGrids)
    px,py=pPoint
    foodGrids[px][py]=False
    dis+=util.manhattanDistance(pPoint,position)
    position=pPoint
  return dis

def disFood (position,foodGrids):
  row = 0
  min = 9999999
  x, y = position
  for p in foodGrids:
      ...
      for d in p:
        if d == True:
          if min > abs(x - row) + abs(y - cob):
            min = abs(x - row) + abs(y - cob)
            point = row, cob
        cob += 1
      row += 1
  return  point
